...
public class VIPCafe {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(calculateServeOrder(priorities, K));
    input.close();
  }

  private static int calculateServeOrder(int[] priorities, int friendIndex) {
    ...
    while (!queue.isEmpty()) {
      ...
      boolean hasHigherPriority = currentOrder.priority < maxPriority;
      ...
      if (hasHigherPriority) {
        ...
        servedCount++;
        if (currentOrder.index == friendIndex) {
          return servedCount;
        }
        maxPriority = Math.max(0, maxPriority - 1);
      }
    }
    return -1;
  }
