...
public class GoodStringDistance {
  ...
  public static void main(String[] args) {
    ...
    String goodChars = input.nextLine();
    String studentName = input.nextLine();

    int[] asciiGoodChars = new int[goodChars.length()];
    for (int i = 0; i < goodChars.length(); i++) {
      asciiGoodChars[i] = (int) goodChars.charAt(i);
    }
    Arrays.sort(asciiGoodChars);

    int totalDistance = 0;
    int lastUsedGoodChar = (int) goodChars.charAt(0);

    for (int i = 0; i < studentName.length(); i++) {
      int currentCharAscii = (int) studentName.charAt(i);
      int left = 0, right = asciiGoodChars.length - 1;
      int minimumDistance = Integer.MAX_VALUE;
      int closestGoodChar = -1;

      while (left <= right) {
        int mid = (left + right) / 2;
        if (asciiGoodChars[mid] == currentCharAscii) {
          closestGoodChar = lastUsedGoodChar;
          minimumDistance = 0;
          break;
        } else if (asciiGoodChars[mid] < currentCharAscii) {
          if (
            Math.abs(asciiGoodChars[mid] - currentCharAscii) < minimumDistance
          ) {
            minimumDistance = Math.abs(asciiGoodChars[mid] - currentCharAscii);
            closestGoodChar = asciiGoodChars[mid];
          } else if (
            Math.abs(asciiGoodChars[mid] - currentCharAscii) == minimumDistance
          ) {
            if (
              Math.abs(lastUsedGoodChar - closestGoodChar) >
              Math.abs(lastUsedGoodChar - asciiGoodChars[mid])
            ) {
              closestGoodChar = asciiGoodChars[mid];
              minimumDistance = Math.abs(
                lastUsedGoodChar - asciiGoodChars[mid]
              );
            }
          }
          left = mid + 1;
      ...
      lastUsedGoodChar = closestGoodChar;
